...
export const fetchMangasEdit = (id, title = '', genres = []) => {
  return async dispatch => {
    ...
    try {
      ...
      const response = await axios.get(`http://localhost:8080/api/mangas/creator/${id}`, { // Usar el id dinámico en la URL
        headers: { 'Authorization': `Bearer ${token}` },
        params: {
          title,
          genres: genres.join(',') // Asegúrate de que los géneros se están enviando correctamente
        }
      });
