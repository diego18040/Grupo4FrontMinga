...
import { fetchMangasEdit, fetchGenresEdit } from '../store/actions/CardsEditActions';

export default function CheckboxsEdit() {
    ...
    useEffect(() => {
        dispatch(fetchGenresEdit());
    }, [dispatch]);

    useEffect(() => {
        setActiveGenres(selectedGenre);
    }, [selectedGenre]);
    ...
    const handleGenreClick = (genre) => {
        ...
        dispatch(fetchMangasEdit(selectedTitle, newActiveGenres));
        dispatch({ type: 'SET_SELECTED_GENRE', payload: newActiveGenres });
    };

    const handleAllClick = () => {
        setActiveGenres([]);
        dispatch(fetchMangasEdit(selectedTitle, []));
